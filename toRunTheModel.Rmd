...
*selPoints*: Fishing selectivity (catchability) from *Kendall & Quin (2013) Oikos*. This implies that larger fishes
can escape the fishe (are less likely to be cougth).

```{r}
F <- 0.7
selectivity <- TRUE #The model is selective
sexComp <- 0.5 
selPoints <- data.frame(pp =   c(0.2, 0.95, .85, 0.7), # Kendall & Quin (2013) Oikos
                        size = c(400, 500, 600, 670))
 
```

## Population fertily by age
...
```{r}
rec <- data.frame( ferti = c(1,  1,  2,  4,  5),
...
AgeComp <- out[[1]]
//# sexComp = 0.5; F = 0.7; runFor = 70; selectivity = T; selPoints=selPoints; ageDistro = ageDistro; FishAges = recruits$age; rec = recruits;  escGoal = 2000
...
age7 <- ggplot(data = filter(AgeComp2, AgeNumeric >= 7), aes(x = Year, y = value, group = AgeNumeric, shape = Age, 
                                                             color = Age)) +
...
fem5 <- out[[2]]
fem5$trunkedLength <- round(fem5$length/10, 0)*10

f5 <- ggplot(fem5, aes(x=trunkedLength))+
  geom_histogram(bins=20) +
  xlim(200, 800) + ylim(0, 1600) +
  facet_wrap(~age, nrow=1) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.background = element_blank())

femE <- out[[3]]
femE$trunkedLength <- round(femE$length/10, 0)*10

fE <- ggplot(femE, aes(x=trunkedLength))+
  geom_histogram(bins=20) +
  xlim(200, 800) + ylim(0, 1600) +
  facet_wrap(~age, nrow=1) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
plot_grid(f5, fE, ncol=1, nrow=2, align = "v")




 ggsave(plot=last_plot(), file="NoSelectivity_Esc2000_ASL.png", 
        path = "~/sasap/github/sasap-data/data-processing/plots_and_analisis/EscapeQuality", 
